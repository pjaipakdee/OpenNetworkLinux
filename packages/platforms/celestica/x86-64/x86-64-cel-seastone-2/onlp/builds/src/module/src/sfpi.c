...
static int 
cel_seastone_2_qsfp_sfp_node_read_int(char *path, int *value, int data_len)
{
    //t = clock();
    int ret = 0;
    char buf[8];    
...
static char* 
cel_seastone_2_sfp_qsfp_get_port_path(int port, char *node_name)
{
    //t = clock();
...
static char* 
cel_seastone_2_sfp_qsfp_get_eeprom_path(int port, char *node_name)
{
    //t = clock();
    if(port <= qsfp_count__ + sfp_count__){
        if(port<=qsfp_count__){
...
static uint64_t
cel_seastone_2_sfp_qsfp_get_all_ports_present(void)
{
    //t = clock();
	int i, ret;
	uint64_t present = 0;
	...
	for(i = 0; i < (qsfp_count__+sfp_count__); i++) {
		path = cel_seastone_2_sfp_qsfp_get_port_path(i + 1, "present");
	    if (cel_seastone_2_qsfp_sfp_node_read_int(path, &ret, 0) != 0) {
	        ret = 0;
	    }
...
onlp_sfpi_is_present(int port)
{
    // t = clock();
    int present;
    char* path = cel_seastone_2_sfp_qsfp_get_port_path(port + 1, "present");
    if (cel_seastone_2_qsfp_sfp_node_read_int(path, &present, 0) != 0) {
        if(port <= qsfp_count__){
...
onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
	...
	presence_all = cel_seastone_2_sfp_qsfp_get_all_ports_present();
    /* Populate bitmap */
...
onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
	...
	path= cel_seastone_2_sfp_qsfp_get_eeprom_path(port + 1, "eeprom");
    ...
    //t = clock() - t;
    //time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds 
    //DEBUG_PRINT("[time_debug][port %d] [%s] took %f seconds to execute \n",port,__FUNCTION__, time_taken);
    return ONLP_STATUS_OK;
}
