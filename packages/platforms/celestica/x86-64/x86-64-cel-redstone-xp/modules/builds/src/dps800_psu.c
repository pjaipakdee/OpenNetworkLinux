...
static struct cel_sysfs_obj *psu_obj;
static int psu_id = 1;
				...
				const struct i2c_device_id *id)
{
	...
	if(psu_id == 1) {
		psu_obj = cel_sysfs_kobject_create(PSU_DIR_NAME, NULL);
		if(!psu_obj) {
			printk("create kobj %s error", PSU_DIR_NAME);
			return -ENODEV;
		}
	}
	memset(psu_name, 0, CEL_NAME_SIZE);
	sprintf(psu_name, "psu%d", psu_id++);
	status = sysfs_create_link(&psu_obj->kobj, &client->dev.kobj, psu_name);
	if (status) {
...
static int dps_800ab_16_d_remove(struct i2c_client *client)
{
	...
	for(i = 1; i < psu_id; i++){
		memset(psu_name, 0, CEL_NAME_SIZE);
		sprintf(psu_name, "psu%d", i);
		sysfs_remove_link(&psu_obj->kobj, psu_name);
	}
	kobject_put(psu_obj);
