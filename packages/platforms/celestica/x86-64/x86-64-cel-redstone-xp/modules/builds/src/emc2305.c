...
static struct cel_sysfs_obj *emc2305_obj;
static int emc2305_idx = 1;
...
static int emc2305_remove(struct i2c_client *client)
{
	struct emc2305_data *data = i2c_get_clientdata(client);
	int fan_idx, i;
	char emc2305_name[CEL_NAME_SIZE] = "\0";

	for(i = 1; i < emc2305_idx; i++){
		memset(emc2305_name, 0, CEL_NAME_SIZE);
		sprintf(emc2305_name, "%d", i);
		sysfs_remove_link(&emc2305_obj->kobj, emc2305_name);
	}
	kobject_put(emc2305_obj);
...
emc2305_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(emc2305_idx == 1) {
		emc2305_obj = cel_sysfs_kobject_create(EMC2305_DIR_NAME, NULL);
		if(!emc2305_obj) {
			printk("create kobj %s error", EMC2305_DIR_NAME);
			return -ENODEV;
		}
	}
	memset(emc2305_name, 0, CEL_NAME_SIZE);
	sprintf(emc2305_name, "%d", emc2305_idx++);
	status = sysfs_create_link(&emc2305_obj->kobj, &client->dev.kobj, emc2305_name);
	if (status) {
