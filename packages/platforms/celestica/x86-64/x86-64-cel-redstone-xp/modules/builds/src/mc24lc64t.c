...
static struct cel_sysfs_obj *eeprom_obj;
                         ...
                         const struct i2c_device_id *id)
{
	...
	eeprom_obj = cel_sysfs_kobject_init();
	if(!eeprom_obj) {
		printk("create kobj %s error", EEPROM_DIR_NAME);
		return -ENODEV;
	}
	err = sysfs_create_link(&eeprom_obj->kobj, &client->dev.kobj, EEPROM_DIR_NAME);
	if (err) {
		printk("Cannot create the link: err=%d\n", err);
		kobject_put(eeprom_obj);
		return -ENODEV;
	}
...
static int mc24lc64t_remove(struct i2c_client *client)
{
	...
	sysfs_remove_link(&eeprom_obj->kobj, EEPROM_DIR_NAME);
	kobject_put(eeprom_obj);
    i2c_unregister_device(drvdata->fake_client);
