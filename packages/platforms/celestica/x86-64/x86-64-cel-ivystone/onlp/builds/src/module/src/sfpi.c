...
static int qsfp_count__ = 128;
static int sfp_count__ = 0;
...
static int 
cel_ivystone_qsfp_sfp_node_read_int(char *path, int *value, int data_len)
{
    int ret = 0;
    char buf[8];    
    *value = 0;
...
static char* 
cel_ivystone_sfp_qsfp_get_port_path(int port, char *node_name)
{
...
static char* 
cel_ivystone_sfp_qsfp_get_eeprom_path(int port, char *node_name)
{
...
{
    int present;
    char* path = cel_ivystone_sfp_qsfp_get_port_path(port + 1, "present");
    if (cel_ivystone_qsfp_sfp_node_read_int(path, &present, 0) != 0) {
        if(port <= qsfp_count__){
...
onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
	...
	path= cel_ivystone_sfp_qsfp_get_eeprom_path(port + 1, "eeprom");
