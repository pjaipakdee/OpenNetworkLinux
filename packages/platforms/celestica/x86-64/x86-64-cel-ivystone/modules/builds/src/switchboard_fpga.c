/*
 * switchboard.c - Driver for Ivystone Switch FPGA/CPLD.
 *
...
#define CLASS_NAME "ivystone_fpga"
#define DRIVER_NAME "ivystone.switchboard"
#define FPGA_PCI_NAME "ivystone_fpga_pci"
...
struct ivystone_fpga_data *fpga_data;
...
static struct device * ivystone_sff_init(int portid) {
...
static const struct i2c_algorithm ivystone_i2c_algorithm = {
 ...
 */
static struct i2c_adapter * ivystone_i2c_init(struct platform_device *pdev, int portid, int bus_number_offset)
{
    int error;
    ...
    new_adapter->algo  = &ivystone_i2c_algorithm;
...
// I/O resource need.
static struct resource ivystone_resources[] = {
    {
        .start  = 0x10000000,
        .end    = 0x10001000,
...
static void ivystone_dev_release( struct device * dev)
{
    return;
}

static struct platform_device ivystone_dev = {
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(ivystone_resources),
    .resource       = ivystone_resources,
    .dev = {
        .release = ivystone_dev_release,
    }
};
...
static int ivystone_drv_probe(struct platform_device *pdev)
{
    struct resource *res;
    int ret = 0;
    int portid_count;
    ...
    fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct ivystone_fpga_data),
                             GFP_KERNEL);
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++) {
        ...
        fpga_data->i2c_adapter[portid_count] = ivystone_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++) {
        ...
        if (i2c_adap) {
            fpga_data->sff_devices[portid_count] = ivystone_sff_init(portid_count);
...
static int ivystone_drv_remove(struct platform_device *pdev)
{
    int portid_count;
...
module_init(ivystone_init);
module_exit(ivystone_exit);
...
MODULE_DESCRIPTION("Celestica Ivystone switchboard platform driver");
