...
static int 
dellemc_z9332f_d1508_qsfp_sfp_node_read_int(char *path, int *value, int data_len)
{
    int ret = 0;
    char buf[8];    
    *value = 0;
...
static char* 
dellemc_z9332f_d1508_sfp_qsfp_get_port_path(int port, char *node_name)
{
...
static char* 
dellemc_z9332f_d1508_sfp_qsfp_get_eeprom_path(int port, char *node_name)
{
...
static uint64_t
dellemc_z9332f_d1508_sfp_qsfp_get_all_ports_present(void)
{
	int i, ret;
	uint64_t present = 0;
    char* path;
	...
	for(i = 0; i < (qsfp_count__+sfp_count__); i++) {
		path = dellemc_z9332f_d1508_sfp_qsfp_get_port_path(i + 1, "present");
	    if (dellemc_z9332f_d1508_qsfp_sfp_node_read_int(path, &ret, 0) != 0) {
	        ret = 0;
	    }
...
{
    int present;
    char* path = dellemc_z9332f_d1508_sfp_qsfp_get_port_path(port + 1, "present");
    if (dellemc_z9332f_d1508_qsfp_sfp_node_read_int(path, &present, 0) != 0) {
        if(port <= qsfp_count__){
...
onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
	...
	presence_all = dellemc_z9332f_d1508_sfp_qsfp_get_all_ports_present();
