...
int is_cache_exist()
{
    ...
    //Read setting
    if (access(time_setting_path, F_OK) == -1)
    { //Setting not exist
        return -1;
    }
    else
...
int is_shm_mem_ready()
{
    ...
    if (access(fru_cache_path, F_OK) == -1 || access(sdr_cache_path, F_OK) == -1)
    { //Shared cache files not exist
        return 0;
    }
...
int get_psu_info(int id, int *mvin, int *mvout, int *mpin, int *mpout, int *miin, int *miout)
{
    ...
    /*
        ...
        PSU1_VIn         | na         | Volts      | na    | na        | na        | na        | 239.800   | 264.000   | na        
        PSU1_CIn         | na         | Amps       | na    | na        | na        | na        | na        | 14.080    | na        
        PSU1_PIn         | na         | Watts      | na    | na        | na        | na        | na        | 1500.000  | na        
        PSU1_Temp1       | na         | degrees C  | na    | na        | na        | na        | na        | na        | na        
        PSU1_Temp2       | na         | degrees C  | na    | na        | na        | na        | na        | na        | na        
        PSU1_VOut        | na         | Volts      | na    | na        | na        | na        | na        | 13.500    | 15.600    
        PSU1_COut        | na         | Amps       | na    | na        | na        | na        | na        | 125.000   | na        
        PSU1_POut        | na         | Watts      | na    | na        | na        | na        | na        | 1500.000  | na        
        PSU2_VIn         | 228.800    | Volts      | ok    | na        | na        | na        | 239.800   | 264.000   | na        
        PSU2_CIn         | 0.480      | Amps       | ok    | na        | na        | na        | na        | 14.080    | na        
        PSU2_PIn         | 114.000    | Watts      | ok    | na        | na        | na        | na        | 1500.000  | na        
        PSU2_Temp1       | 26.000     | degrees C  | ok    | na        | na        | na        | na        | na        | na        
        PSU2_Temp2       | 43.000     | degrees C  | ok    | na        | na        | na        | na        | na        | na        
        PSU2_VOut        | 12.000     | Volts      | ok    | na        | na        | na        | na        | 13.500    | 15.600    
        PSU2_COut        | 7.500      | Amps       | ok    | na        | na        | na        | na        | 125.000   | na        
        PSU2_POut        | 90.000     | Watts      | ok    | na        | na        | na        | na        | 1500.000  | na        
        root@localhost:~# 
    */
...
int open_file(const char *shm_path, const char *sem_path, char **cache_data, int *cache_size)
{
    ...
    if (!res)
    {
        tmp_ptr = malloc(shm_map_tmp.size + 1);
        if (!tmp_ptr)
        {
            res = -1;
            return res;
        ...
        tmp_ptr[shm_map_tmp.size] = '\0';
        
        *cache_data = tmp_ptr;

        *cache_size = shm_map_tmp.size + 1;
    }
